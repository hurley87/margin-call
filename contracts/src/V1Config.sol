...
/// @dev Stock-specific token/feed/pool addresses live only in `LaunchAssets` (tests/scripts). This library
///      owns chain infrastructure, risk pins, and the shared 8/8/6 valuation invariant — not per-stock rails.
library V1Config {
    ...
    /// @dev Protocol-wide decimal invariant for every launch-set stock and its total-return feed.
