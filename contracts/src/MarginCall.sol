...
///      permissionlessly unwinds unhealthy financed positions under LIVE pricing. `riskSnapshot` is the LIVE-only
///      risk view (nav, current debt, liquidatable) sharing the liquidation predicate. Borrowed USDC can only buy
...
contract MarginCall is ERC721 {
    ...
    ///      rule, shared by financed opening, `reduceExposure`, `liquidate`, and `riskSnapshot` so none can drift
    ///      from the others.
