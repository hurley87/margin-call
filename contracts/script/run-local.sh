#!/usr/bin/env bash
# Local Anvil-only smoke harness for the spot Position NFT lifecycle.
...
echo "Running spot Position NFT signer smoke test against local Anvil (${RPC_URL}, chain ${chain_id})"
echo "Private key is read from MARGIN_CALL_PRIVATE_KEY and is never printed."
echo "Broadcast artifact: broadcast/SpotPositionLifecycle.s.sol/31337/run-latest.json"

exec forge script script/SpotPositionLifecycle.s.sol:SpotPositionLifecycle \
  --rpc-url "$RPC_URL" \
  --broadcast \
  -vv
