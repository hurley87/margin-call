...
///      deploys the production adapter stack against the real token and feed addresses, wires and seeds
///      `CreditPool`, and funds one actor with NVDAc. Mirrors the role `MarginCallTestBase` plays for the
///      RPC-free suites so a constructor change to any of the four contracts is a single edit here.
///      Subclasses supply their own actor label via `_forkActorLabel()` and add only their assertions.
