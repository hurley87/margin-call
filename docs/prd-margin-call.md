...
8. **Maker Emissions** accrue continuously on-chain (hard-capped by the Distributor's funded balance) and are claimable at any time; **Participation Rewards** accrue per closed daily epoch as an equal-per-successful-Rip share of that day's pot. There is no Season; the game runs open-ended, and redemption is always available.
...
On-chain, a Maker's total is the sum of **checkpointed** Pack NAVs — recorded at enrollment and re-read by a permissionless `syncPackNav` after a top-up or a price move — because grouping a live NAV read by Maker inside a Rip would need an unbounded pass over the resting set. A checkpoint refresh fails closed on a bad feed, so an oracle gap can never be used to zero a rival's total. The reigning Maker keeps the crown while their total shrinks and vacates it at zero resting NAV; a vacant crown carves nothing and is picked up by the next Maker to challenge, and `challengeCrown` lets a standing Maker press a claim after the leader shrinks without touching the pool.
...
**Emission is a funded Distributor, not a mint.** The GameToken is a plain fixed-supply ERC-20, **fully minted at deploy to the treasury — there is no ongoing emission-mint authority.** The owner **funds a Distributor contract by transferring tokens into it** and sets the stream rates on-chain; it pays out only tokens it holds. This makes **the Distributor's balance the hard cap by construction**: underfunding reverts a claim without consuming it (a later top-up recovers), and there is no mint path to inflate supply.

- **Maker Emissions** — a continuous on-chain stream from the Distributor to Makers, **equal per resting Pack** in V1, at an **owner-settable rate** (`makerRatePerEpoch`, converted to a per-second rate). The RipEngine checkpoints accrual on Pack enter / exit / Rip; Makers claim at any time. The gap-weighted **restock controller** (`∝ gap^convexity ÷ inventory`, per-asset `target_inventory`, `gain ≈ 8`, `convexity = 2`) that steers composition is a **post-V1** mechanism — no economic pull while the token is transfer-locked and the House manages composition.
- **Participation Rewards** — a **fixed daily token pot** (`takerPotPerEpoch`) split **equally across successfully ripped Packs** in that epoch (epoch = 1 day, anchored to Distributor deploy time). A batch of `count` Packs contributes `count` Rips to both the Taker's and the epoch's totals. Because the pot is fixed, a quieter epoch returns a larger rebate per Rip automatically — the "quieter pool → bigger rebate" property emerges for free, no activity metric needed. There is **no per-Rip cap and no rollover**: an empty epoch creates no liability, and floor-division dust stays in the Distributor's funded balance. Paid in game token, entirely separate from the stablecoin surcharge flow, so Model A is untouched; the rebate simply softens the Taker's −EV in token.
- **Claims** — both streams are accounted **fully on-chain**. Maker Emissions use a per-Pack accumulator (claimable continuously). Participation Rewards use per-epoch Rip counts (claimable once the epoch has closed). Anyone can read the same state the contracts use; there are no merkle Claim Roots and no off-chain entitlement builder.
...
| `makerRatePerEpoch` / `takerPotPerEpoch`           | Distributor stream rates, owner-settable; funded by transferring tokens in. Maker rate is continuous; Taker pot is equal-per-Rip per closed epoch | TBD                                              |
...
- Settlement conserves the full payment: `protocol_cut` and `crown_cut` from the surcharge only, remainder socialized equally across resting Packs; the base is never touched by any cut, so make-whole holds for every `protocolShareOfSurcharge + crownShareOfSurcharge ≤ 1`. The Crown pays the Maker with the largest total resting NAV and only changes hands when a challenger beats the leader by ≥ `crownBeatMargin` — never on a tie, so it cannot flicker — and is vacated, carving nothing, by a Maker with nothing left resting. The drawn Pack and its full basket transfer to the Taker.
...
- Game-token payouts can never exceed the Distributor's funded balance — it pays only tokens it holds and there is no emission-mint authority. Maker Emissions accrue continuously on-chain equal per resting Pack; Participation Rewards split a fixed daily pot equally across successfully ripped Packs in a closed epoch (a batch of `count` counts as `count` Rips). Empty epochs create no liability; floor-division dust stays in the funded balance. An underfunded claim reverts without consuming the entitlement.
...
- Maker-emission and participation-pot accounting are fully on-chain (continuous equal-per-Pack Maker accrual; equal-per-successful-Rip closed-epoch Taker claims) and stay within the Distributor's funded balance;
...
- Token supply/funding have recommended starting points (1B supply, ~30% funded to the Distributor, ~60/40 Taker-weighted); still open are the exact `makerRatePerEpoch` / `takerPotPerEpoch` and the intended stream horizon. _(Surcharge split, Crown values, and Participation Rewards as equal-per-successful-Rip daily pots with no cap/rollover are now pinned — see the owner control panel and Game Token section.)_
