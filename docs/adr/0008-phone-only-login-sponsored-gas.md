...
2. **Gasless needs this anyway.** Sponsorship requires smart accounts and a paymaster; embedded wallets are the shortest path. Entry uses a one-time bounded `1,000 tUSD` approval when needed, then sponsored enter-only transactions — collapsing gas UX without requiring an atomic approve+enter batch in the React SDK.
...
- Player entry uses a one-time bounded `1,000 tUSD` allowance to `BankrollVault` when needed, then sponsored enter-only transactions. The React Privy client does not perform atomic approve+enter batching; unlimited allowance is never requested.
