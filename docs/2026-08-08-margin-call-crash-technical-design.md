...
- Constrain the paymaster sponsorship policy with contract **and function-selector** allowlists plus per-operation, per-address, and global spend limits; keep the paymaster endpoint out of the client bundle (server-proxied); monitor spend. A drained, exhausted, or misconfigured policy must degrade to a clear user-facing error with retry, never to silent failure, and negative tests cover unapproved addresses, unapproved selectors, exhausted limits, and policy outage.
