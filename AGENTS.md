...
Margin Call is a Next.js 16 (App Router) web app — an AI-powered PvP trading game on 1980s Wall Street. See `CLAUDE.md` for tech stack, commands, and architecture details. See `docs/wall-street-agent-game.md` for the full game design spec. Floor chain/intent boundaries: `docs/floor/chain-and-intent-boundaries.md` (Robinhood Chain testnet only). Legacy Base Sepolia address records: `docs/base-sepolia-configuration.md` / `convex/lib/legacy`.
