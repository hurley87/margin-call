...
describe("calculateCreationFee", () => {
  ...
  it("calculates 5% fee on 20 USDC (minimum)", () => {
    expect(calculateCreationFee(20)).toBe(1);
  });
...
describe("calculateNetPot", () => {
  ...
  it("returns pot minus fee for minimum amount", () => {
    expect(calculateNetPot(20)).toBe(19);
  });
});
...
describe("resolveDealPaymentPrice", () => {
  ...
  it("returns price for minimum pot amount", () => {
    expect(resolveDealPaymentPrice({ pot_amount: 20 })).toBe("$20.00");
  });
  ...
  it("returns null for pot_amount below minimum", () => {
    expect(resolveDealPaymentPrice({ pot_amount: 5 })).toBeNull();
  });
