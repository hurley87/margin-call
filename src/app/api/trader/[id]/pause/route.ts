...
export async function POST(
  ...
  try {
    ...
    if (trader.status === "paused") {
      return NextResponse.json({ message: "Trader is already paused", trader });
    }
    if (trader.status === "wiped_out") {
      return NextResponse.json(
        { error: "Cannot pause a wiped out trader" },
        { status: 400 }
      );
    }
    ...
    await logActivity(
      id,
      "pause",
      `Trader "${trader.name}" paused by desk manager`
    );

    return NextResponse.json({ message: "Trader paused", trader: updated });
  } catch (e) {
