...
import { useCrashTicketRefund } from "./use-crash-ticket-refund";
...
describe("useCrashTicketRefund", () => {
  ...
  it("loads a returning player's expired ticket and refunds margin", async () => {
    const { result } = renderHook(() => useCrashTicketRefund());

    await waitFor(() => expect(result.current.status).toBe("ready"));
    expect(result.current.outcome).toBe("refundable");
    expect(result.current.canRefund).toBe(true);
    expect(result.current.payout).toBe(5_000_000n);
  ...
  it("marks an expiry-eligible round expired", async () => {
    ...
    sdk.readPlayerRecentTicket.mockResolvedValue({
      ...
      round: {
        ...expiredRound,
        status: 1,
        expiresAt: BigInt(now - 10),
      },
    });
    const { result } = renderHook(() => useCrashTicketRefund());
    await waitFor(() => expect(result.current.canExpire).toBe(true));

    await act(async () => {
      await result.current.expireRound();
    });
